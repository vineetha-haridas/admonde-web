...
function BrandLogo({ name, logoUrl }: BrandClient) {
  ...
  return (
    <div
      className="shrink-0 flex items-center justify-center w-44 h-20 3xl:w-52 3xl:h-24"
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      {errored ? (
        <span
          className="text-[16px] font-bold tracking-widest uppercase select-none"
        ...
        <img
          src={logoUrl}
          alt={name}
          className="object-contain max-h-18 3xl:max-h-22 w-auto transition-all duration-300"
          style={{
...
function MarqueeSkeleton() {
  return (
    <div className="flex items-center gap-24 flex-1 min-w-0 overflow-hidden px-2">
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          key={i}
          className="shrink-0 h-14 rounded-md bg-[#E8E4DC] animate-pulse"
          style={{ width: `${[130, 158, 114, 174, 142, 122][i]}px`, opacity: 1 - i * 0.12 }}
        />
      ))}
    </div>
  );
  ...
  return (
    <div className="overflow-hidden flex-1 min-w-0">
      <div
        ...
        style={{ animationDuration: `${Math.max(20, clients.length * 2.5)}s`, gap: "6rem" }}
      >
        {looped.map((b, i) => (
